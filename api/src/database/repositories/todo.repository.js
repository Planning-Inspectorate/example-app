...
export function addTaskData (taskDeadlineFriendly, title, description, priority) {
    return connection.query(
        'INSERT INTO to_do (completeBy, title, description, priority) VALUES (?, ?, ?, ?)', [taskDeadlineFriendly, title, description, priority]
    );
}

export function deleteTaskData (taskId) {
    return connection.query(
        'DELETE FROM to_do WHERE id = ?', taskId
    );
}
...
export function getTaskData (taskId) {
    return connection.query(
        'SELECT * FROM to_do WHERE id = ?', taskId
    );
}
